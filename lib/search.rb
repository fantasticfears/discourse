...
class Search
  ...
  def self.ts_config(locale = SiteSetting.default_locale)
    ...
    case locale.to_sym
      ...
      when :zh_CN  then 'jiebacfg'
      when :zh_TW  then 'jiebacfg'
      else 'simple' # use the 'simple' stemmer for other languages
    end
  end

  def self.ts_query_config(locale = SiteSetting.default_locale)
    # same rule with #ts_config.
    # but this is for generate the vector for TO_TSQUERY.
    case locale.to_sym
      when :zh_CN  then 'jiebaqry'
      when :zh_TW  then 'jiebaqry'
      else 'simple'
    end
  end
