...
class ImportScripts::DiscuzX < ImportScripts::Base
  ...
  def initialize
    ...
    @internal_url_regexps = [
      ...
      /http(?:s)?:\/\/#{ORIGINAL_SITE_PREFIX.gsub('.', '\.')}\/thread-(?<tid>\d+)-(?:[^\[\]\s]*)/i,
      /http(?:s)?:\/\/#{ORIGINAL_SITE_PREFIX.gsub('.', '\.')}\/forum-(?<fid>\d+)-(?:[^\[\]\s]*)/i
    ]
  ...
  def get_knowledge_about_duplicated_email
    @duplicated_email = {}
    member_table = table_name 'common_member'

    results = mysql_query("
        SELECT a.uid uid,
               b.uid import_id
          FROM #{member_table} a
          JOIN (SELECT uid, email FROM #{member_table}
                 GROUP BY email HAVING COUNT(email) > 1
                 ORDER BY uid asc) b USING(email)
         WHERE a.uid != b.uid;")
    ...
    batches(BATCH_SIZE) do |offset|
      results = mysql_query(
        "SELECT u.uid id, u.username username, u.email email, u.groupid group_id,
                    su.regdate regdate, su.password password, su.salt salt,
    ...
    batches(BATCH_SIZE) do |offset|
      results = mysql_query("
            SELECT p.pid id,
                   ...
                   t.views views
              FROM #{posts_table} p
              JOIN (SELECT pid, tid FROM #{posts_table} WHERE first) p2 USING(tid)
              JOIN (SELECT fid, subject, special, views FROM #{topics_table}) t USING(tid)
  ...
  def replace_internal_link(discuzx_link, import_topic_id, import_post_id, import_category_id, action)
    ...
    if import_topic_id
      ...
      if lookup = topic_lookup_from_imported_post_id(linked_post_id)
  ...
  def import_attachments
    ...
    Post.find_each do |post|
      ...
      if new_raw != post.raw
        begin
          PostRevisor.new(post).revise!(user, { raw: new_raw }, { bypass_bump: true, revised_at: post.last_version_at || Time.now, bypass_rate_limiter: true, edit_reason: '从 Discuz 中导入附件' })
        rescue
          p "PostRevisor Error: #{post} #{user}"
          fail_count += 1
          next
        end
      end
  ...
  def mysql_query(sql)
    ## TODO: MySQL gem bug, preventing segment fault
